...
const prospectosSlice = createSlice({
  ...
  reducers: {
    ...
    updatePersonalInfo: (state, action: PayloadAction<Partial<PersonalInfoState>>) => {
      state.form.personalInfo = { ...state.form.personalInfo, ...action.payload }
    },
    ...
    // Address
    updateFiscalAddress: (state, action: PayloadAction<Partial<AddressData>>) => {
      state.form.address.fiscal = { ...state.form.address.fiscal, ...action.payload }
    },
    updateCorrespondenciaAddress: (state, action: PayloadAction<Partial<AddressData>>) => {
      state.form.address.correspondencia = { ...state.form.address.correspondencia, ...action.payload }
    },
    copyFiscalToCorrespondencia: (state) => {
      state.form.address.correspondencia = { ...state.form.address.fiscal }
    },
    ...
    updateContactInTable: (state, action: PayloadAction<ContactTableItem>) => {
      const index = state.form.contactsTable.findIndex((c) => c.id === action.payload.id)
      if (index !== -1) {
        state.form.contactsTable[index] = action.payload
      }
    },
    deleteContactFromTable: (state, action: PayloadAction<string>) => {
      state.form.contactsTable = state.form.contactsTable.filter((c) => c.id !== action.payload)
    },
    ...
    updateHRFormData: (state, action: PayloadAction<Partial<HRFormData>>) => {
      state.form.hr.formData = { ...state.form.hr.formData, ...action.payload }
    },
    ...
    deleteHRRecord: (state, action: PayloadAction<string>) => {
      state.form.hr.records = state.form.hr.records.filter((r) => r.id !== action.payload)
    },
    ...
    updateNafinsa: (state, action: PayloadAction<Partial<ExternalEntitiesState['nafinsa']>>) => {
      state.form.externalEntities.nafinsa = { ...state.form.externalEntities.nafinsa, ...action.payload }
    },
    ...
    updateBuroRecord: (state, action: PayloadAction<BuroRecord>) => {
      const index = state.form.externalEntities.buroCreditos.findIndex((r) => r.id === action.payload.id)
      if (index !== -1) {
        state.form.externalEntities.buroCreditos[index] = action.payload
      }
    },
    deleteBuroRecord: (state, action: PayloadAction<string>) => {
      state.form.externalEntities.buroCreditos = state.form.externalEntities.buroCreditos.filter(
        (r) => r.id !== action.payload,
      )
    },

    // External Entities - CNBV
    updateCNBV: (state, action: PayloadAction<Partial<ExternalEntitiesState['cnbv']>>) => {
      state.form.externalEntities.cnbv = { ...state.form.externalEntities.cnbv, ...action.payload }
    },

    // External Entities - Banxico
    updateBanxico: (state, action: PayloadAction<Partial<ExternalEntitiesState['banxico']>>) => {
      state.form.externalEntities.banxico = { ...state.form.externalEntities.banxico, ...action.payload }
    },

    // External Entities - Infonavit
    updateInfonavit: (state, action: PayloadAction<Partial<ExternalEntitiesState['infonavit']>>) => {
      state.form.externalEntities.infonavit = { ...state.form.externalEntities.infonavit, ...action.payload }
    },
    ...
    updateInfonavitRecord: (state, action: PayloadAction<InfonavitRecord>) => {
      const index = state.form.externalEntities.infonavitRecords.findIndex((r) => r.id === action.payload.id)
      if (index !== -1) {
        state.form.externalEntities.infonavitRecords[index] = action.payload
      }
    },
    deleteInfonavitRecord: (state, action: PayloadAction<string>) => {
      state.form.externalEntities.infonavitRecords = state.form.externalEntities.infonavitRecords.filter(
        (r) => r.id !== action.payload,
      )
    },
